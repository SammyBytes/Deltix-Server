...
| Transport security for the ticket-authenticated channel (gRPC) | Done | The gRPC Transfer Engine (`Push`/`Pull`/`Heartbeat`) is now live and always bound with TLS server credentials (`bindGrpcTransferServer` — no plaintext code path exists, cert/key paths are mandatory env vars `DELTIX_GRPC_TLS_CERT_PATH`/`DELTIX_GRPC_TLS_KEY_PATH`). The first message of every `Push` call must be a `Header` carrying a valid, unconsumed ticket scoped to the exact `(operation, repo)`; any other message first, or an invalid/expired/mismatched/already-consumed ticket, fails closed with `PERMISSION_DENIED`/`FAILED_PRECONDITION` — no chunk is ever written to staging without a successfully consumed ticket. Verified end-to-end via 9 real-TLS gRPC integration tests plus a real-subprocess smoke test (`tests/smoke/grpc-transfer-boot.smoke.test.ts`) exercising the full pipeline: REST ticket issuance -> real TLS gRPC Push -> SSD staging -> background NAS sync worker -> real TLS gRPC Pull, with byte-for-byte checksum verification. `scripts/generate-dev-tls-certs.ts` generates a self-signed DEV-ONLY cert for local testing; production MUST supply a real CA-signed cert via env vars, never a generated one. |
| Push message ordering / callback-timing races (V11.1) | Done | Two real concurrency bugs were found and fixed via TDD: (1) grpc-js delivers client-streaming `data` events synchronously/back-to-back while header ticket-activation is async — fixed by serializing all message processing through an internal promise chain so messages are always handled in strict arrival order; (2) calling the RPC's unary `callback()` for an abort detected mid-stream (before the client half-closes) races with in-flight HTTP/2 frames and can silently drop the connection instead of delivering the intended error status — fixed by deferring `callback()` until the stream's `end` event has actually fired. Both fixes are covered by the "reuse a consumed ticket" and "chunk before header" integration tests. |
| Staging area / NAS write isolation | Done (simulated NAS) | Implemented as a separate `TransferJob` state machine in `contexts/storage` (`STAGED -> SYNCING -> SYNCED \| SYNC_FAILED(retry w/ backoff) -> DEAD_LETTER`), enforcing the guardrail "PROHIBIDO EL ACCESO DIRECTO AL NAS" — writes only ever happen through `NasAdapter.copyToNas()` (copy to temp -> verify checksum -> atomic rename), never a direct in-place write. No physical NAS is available in this environment, so `LocalFsNasAdapter` simulates one via a local folder; the contract is identical to what a real NAS client would need to honor, so swapping in a real adapter later requires no changes to `NasSyncService`. Verified under real concurrent load (20 parallel workers racing `claimNextEligible()`/`markFailed()` against the same libSQL file — exactly one winner every time) and via a real-subprocess smoke test exercising the full staged -> synced and staged -> dead_letter -> manually-requeued paths end to end. |
| Orphaned ticket cleanup | Partial | `reapExpired()` exists and is unit/integration tested, but is not yet wired to a scheduled boot-time job — still a follow-up, independent of the gRPC engine now being live. |
| Dead-letter jobs never silently dropped (V11 business logic integrity) | Done | Once `DELTIX_TRANSFER_JOB_MAX_RETRIES` is exhausted a job escalates to `dead_letter` (never deleted); `GET/POST /api/v1/storage/transfer-jobs/dead-letter*` (JWT-protected) expose it for mandatory manual operator review/retry, ready to back a future Admin Web UI panel. |
